...
unsigned int ORBSlamPython::getNumFrames() const
{
    if (system)
    {
...
unsigned int ORBSlamPython::getNumTrackedMapPoints() const
{
    if (system)
    {
        return system->GetTrackedMapPoints().size();
    }
    return 0;
}
...
boost::python::dict ORBSlamPython::getKeyframeState() const
{
    ...
    unsigned int n_frames = getNumFrames();
...
bool ORBSlamPython::getKeyframeNeeded() const
{
    ...
    return system->SLAMNeedNewKeyFrame;
}
